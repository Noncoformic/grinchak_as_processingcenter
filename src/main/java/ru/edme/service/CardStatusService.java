...
public class CardStatusService {
  ...
  public CardStatus getCardStatusByName(String cardStatusName) {
    try {
      Optional<CardStatus> existingStatus =
          cardStatusDao.getAll().stream()
              .filter(status -> status.getCardStatusName().equals(cardStatusName))
              .findFirst();
    ...
    try {
      ...
      if (optionalCardStatus.isPresent()) {
        ...
        CardStatus updatedCardStatus =
            cardStatus.toBuilder().cardStatusName(cardStatusName).build();
        cardStatusDao.update(updatedCardStatus);
        logger.info(
            "🔄 CardStatus updated: {} to {}", cardStatus.getCardStatusName(), cardStatusName);
      }
    } catch (RuntimeException e) {
